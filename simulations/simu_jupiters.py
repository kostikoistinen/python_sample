...
#Simulation: Jupiter bombing

#Classes------------------------------------------------
Au = 150e9 #Distance to the Sun from Earth.
numcom = 10 # Number of comets to be generated
class objectlist:
    name          = ""
    mass          = 0 #kg
...
i=0 #Make comets
jupiters = np.array([objectlist() for _ in range(numcom)])
while i<len(jupiters):
    jupiters[i].name = "jupiter"+str(i)
    jupiters[i].mass = Jupiter.mass
    jupiters[i].distance = [random.choice([-1, 1])*np.random.uniform(30,45)*Au,
                         random.choice([-1, 1])*np.random.uniform(-5,5)*Au]

    jupiters[i].velocity = [0,np.random.uniform(-1000,1000)] #small velocities far away
    jupiters[i].track = copy.deepcopy(jupiters[i].distance)
    jupiters[i].size = Jupiter.size
    jupiters[i].radius = Jupiter.radius
    jupiters[i].color = Jupiter.color
    i=i+1
...
print("Simulation: Comets are jupiters")
l_obj = [Sun,Earth,Mars,Jupiter,Saturn,Uranus,Neptune]
#add comets:
for i in range(numcom):
    l_obj = np.append(l_obj,jupiters[i])
table = PrettyTable()
